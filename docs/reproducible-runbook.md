...
Current runtime evidence as of `20260702T111438Z-vp-modern-runtime`: the VP
boots Linux 6.6, `opendla.ko` loads, `/dev/dri/renderD128` appears, the runtime
server and client complete, and the SDP workload reaches `Test PASSED` from the
server. The correctness gate still fails because the returned
`o_000000.dimg` differs from the pinned golden at byte offset 36 and the tensor
payload is largely zero. Treat this as the next driver/memory correctness bug,
not as a comparator tolerance issue.
