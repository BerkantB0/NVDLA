...
class SdpDiagnosticTests(unittest.TestCase):
    def test_accepts_known_sdp_zero_output_golden_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            artifact = root / "20260709T000000Z-vp-modern-runtime"
            output = artifact / "runtime-output" / "o_000000.dimg"
            output.parent.mkdir(parents=True)
            output.write_bytes(b"DIMG" + bytes(36) + bytes(64))
            (artifact / "manifest.json").write_text(
                f"""{{
  "run_id": "20260709T000000Z-vp-modern-runtime",
  "status": "fail",
  "modern": {{
    "mode": "runtime",
    "status": "fail",
    ...
    "statuses": {{
      ...
      "dev_dri": 0,
      "runtime_client": 0,
      "runtime_server_ready": 0
    }},
    "workloads": [
      {{
        "name": "sdp_regression_small",
        "status": "fail",
        "output_sha256": "zero-payload",
        "compare": {{
          "actual_path": "{output}",
          "reason": "files differ",
          "status": "fail"
...
""",
            ...
            (artifact / "runtime-client.log").write_text(
                "run_result=[OK] Test PASSED!\n",
                encoding="utf-8",
            )
            ...
            self.assertEqual(result["classification"], "known_sdp_zero_output_golden_mismatch")
            self.assertTrue(result["output_summary"]["payload_is_all_zero"])
